...
<div class="w1100">
			...
			<tbody>
				<c:forEach items="${list}" var="list">
					<tr>
						...
						<td><a href="notice_details_page?NO=${list.NO}">${list.TITLE}</a></td>
	...
	<c:if test="${userData.STATUS eq 'A'}">
		<div class="m-t-23">
			<a href="notice_write_page" class="write_btn">글쓰기</a>
		</div>
	</c:if>
	...
	<div class="text-center m-t-30 m-b-60">
		<c:if test="${paging.NOW_PAGE > 1}">
			<a href="notice_page?page_num=${paging.NOW_PAGE-1}" class="m-r-20">이전</a>
		</c:if>
		<div class="paging_box">
			<c:forEach items="${list}" var="list" begin="1" end="${paging.TOTAL_PAGE}" varStatus="status">
				<c:if test="${paging.NOW_PAGE eq status.count}">
					<a href="notice_page?page_num=${status.count}" class="sp_active">${status.count}</a>
				</c:if>
				<c:if test="${paging.NOW_PAGE ne status.count}">
					<a href="notice_page?page_num=${status.count}">${status.count}</a>
				</c:if>
		    </c:forEach>
		</div>
		<c:if test="${paging.NOW_PAGE < paging.TOTAL_PAGE}">
			<a href="notice_page?page_num=${paging.NOW_PAGE+1}" class="m-l-20">다음</a>
		</c:if>
	</div>
	<!-- 페이징 [e] -->
</div>